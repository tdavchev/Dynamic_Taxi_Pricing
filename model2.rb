#bids the optimal value only, but does not have to bid
require 'finite_mdp'
require './taxiSystem.rb'

class OptionalOptimalBidModel
...
def actions(state)
  ...
  n.each do |t|
    ...
    # old
    #if (state[2]!=nil)
    #    actions<<[:bid,t,state[2],@g.optbid(state[1],state[2])] #bid, where in case of fail, where in case of success, amount
    #end
    #
    #revised
    if (state[2]!=nil)
...
# revised
    if state[0] ==:reward && action[0] ==:virtualAction
      ...
      if next_state[0] ==:reward
        return @g.df(action[3]/5, state[1], state[2])  # more change to have a successful bid
